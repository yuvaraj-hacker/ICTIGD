      ...
      <section
        ...
        <header className="max-w-[90rem] mx-auto md:px-5 px-2 h-full py-3 w-full">
          <div className="md:flex md:justify-between justify-center items-center ">
            ...
            <nav ref={dropdownRef}>
              <div className={`space-x-1 md:block  mx-auto  md:pt-0  pt-20  ${menuOpen ? "flex flex-col space-y-4" : ""}`}>
                {navLinks.map((link) => (
                  <div key={link.to} className=" inline-block group relative"
                    ...
                    <Link to={link.to} className={`py-2 flex lg:w-[144px] md:w-[130px] w-full transition-all duration-300 ease-in-out lg:text-base md:text-sm justify-center items-center gap-2 bg-gradient-to-b from-[#2487DB] to-[#014BBD] rounded-full text-white ${isActive(link.to) || hoveredCategory === link.label ? 'md:border-t-0 border-t-[#14AE5C] duration-300 text-[#1B1F3B]' : ''} `}
                      ...
                      {link.label}
                      {link.dropdown && (<i className={`fi fi-sr-angle-circle-down flex items-center  transition-all duration-300 ease-in-out  rounded-full text-white ${ hoveredCategory === link.label ? 'bg-[#14AE5C] text-white rotate-180 duration-100' : ' '} `}></i>)}
                    </Link>
                    {hoveredCategory === link.label && link.dropdown && (
                      <div className="md:absolute left-0 top-full md:w-[260px] w-full rounded-lg md:bg-gradient-to-b from-[#2487DB] to-[#014BBD]   transition-all duration-300 ease-in-out opacity-100 scale-y-100 origin-top  grid md:grid-cols-1 md:p-3 z-10">
                        {link.dropdown.map((dropdownlink) => (
